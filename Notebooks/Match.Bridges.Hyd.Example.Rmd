...
RUN   <- c(FALSE, rep(TRUE, 6))   # Which of the steps above to be run?
...
# Matching options
M_STYLE <- "PAR"      # PAR: Runs for each M_TYPE in parallel (i.e.,
...
M_TYPE <- c("road",   # Determines how data should be matched, and supports: 
            ...
            "bin")
M_STORE <- FALSE      # List of potential matches for each entry is stored on disk,
...
invisible(lapply(unlist(inputs[c("Fail","NBI","Dictionary","Ontologies","StdData")]), load, .GlobalEnv))
...
```{r Example, include = FALSE, echo = FALSE}
...
if (RUN["MATCH"]){
  ...
  ls.Matches <- list()
  for (i in M_TYPE){
    ls.Matches[[i]] <- Match(df.Fail, df.NBI, i, M_STYLE, STORE = M_STORE,
                             STORE_PATH = fp)
  }
