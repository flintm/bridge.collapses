...
opts <-list(Fields   =list(Fail =c("MAT", "TYPE", "LOCATION", "STREAM", "BIN", "ID"),
            ...
            Match=list(maxStringDist =c(road =1, stream =2, route =0, bin = 2),
...
if (RUN["CONFIRM"]){
  ...
  ls.Conf <- lapply(row.names(df.Fail), 
                    function(i)
                      Confirm.Matched.Bridges(df.Fail[i,], df.NBI, ls.Matches.Comb[[i]]))
  tab<-lapply(1:length(ls.Conf),
              function(i)
                formattable(ls.Conf[[i]][1:10,],
                       list(Q_BIN = FALSE, Q_STREAM = FALSE, Q_COUNTY = FALSE,
                            Q_ROAD = FALSE, Q_ROUTE = FALSE, Q_ALL = FALSE,
                            BIN_NBI=formatter("span",
                                              style = ~ style(color = 
                                                                ifelse(Q_BIN<1,
                                                                       "white",
                                                                       "black"),
...
```
# Conclusions
More work is needed, especially in match ranking and confirmation.
