...
if (min(df$SCALE)!=max(df$SCALE)){ # if size scaling is used
  ...
  p <- p +
    scale_size(range = c(r1=sizesP$FailCorrMin[outputType],r2=SCALE_CORRECTION*sizesP$FailCorrMax[outputType]), 
...
#   p <- p + scale_size(max_size = 0.4*sizesP$FailCorr[outputType])
...
p <- p +
  theme(panel.background   = element_rect(fill = "white", color = "gray",size=0.25),
        ...
        panel.spacing  = unit(c(0.5,0,0,0),"in"),
