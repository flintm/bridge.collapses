Match_Failed_Bridges <- function(FailRowsIn = NA, nbiRowsIn = NA, FailDataSeqBy = 1, nbiDataSeqBy = 1){
 ...
 #
 # To-do list:
  #     (1) write code for step (6), (7) - if necessary
  #     (2) see how many matches were made to figure out how should proceed
...
# EXTERNAL FUNCTION CALLS, LIBRARIES, AND DATA--------------------------------------------------------------------------- 
  ...
  # function MatchEntry is a wrapper function that passes failure and nbi data to PerformMatch & stores results
  # function PerformMatch matches strings from the failure entry to the provided NBI data, records match rows
...
#   source('~/Documents/R/clean.df.strings.R')
  source('~/Documents/Research/Climate_Scour_Local/R_data/20150106_MatchEntry.R')
  source('~/Documents/Research/Climate_Scour_Local/R_data/20150106_PerformMatch.R') 
...
#  df.States <- read.csv(paste(DataDir,"StateData.csv", sep = .Platform$file.sep), header = TRUE, 
#                                sep = ",", quote = "\"", dec = ".", 
#                               colClasses = c("character", "character", "integer", "integer"),
#                               col.names = c("STATE_CODE", "STATE_FULL", "STFIPS", "NBI_STATE")) 

 # COUNTY NAMES, FIPS CODES, AND STATES
#  df.Counties <- read.csv(paste(DataDir,"CountyData.csv", sep = .Platform$file.sep), header = TRUE,
#                                sep  = ",", quote = "\"", dec = ".",
...
# df.Cities <- read.csv(paste(DataDir,"CityData.csv", sep = .Platform$file.sep), header = TRUE, 
#                       sep = ",", quote = "\"", dec = ".", 
...
ScreenPrintInterval <- 20;
ScreenPrintSeqBy <- floor(nFail/ScreenPrintInterval)
ScreenPrintSequence <- seq(1,nFail,by=ScreenPrintSeqBy)
ScreenPrintSequence <- c(ScreenPrintSequence, nFail+1)
nIntervals <- length(ScreenPrintSequence)-1
for (interval in 1:nIntervals){
  mclapply(FailRowNames[ScreenPrintSequence[interval]:(ScreenPrintSequence[interval+1]-1)], function(i) MatchEntry(FailDataFrame[i,c("ID","STFIPS", "FIPS", "FIPS_2", "FIPS_FROM_CITY", "FIPS_FROM_CITY_2", "FIPS_FROM_CITY_3", "LOCATION", "ROAD_NAME", "ROAD_TYPE", "ROUTE_NO", "ITEM5B")], 
                                              nbiDataFrame[,c("STFIPS", "FIPS", "ITEM7", "ITEM5B", "ITEM5D")], "road", FALSE, NA), mc.cores = 3)
  print(paste("finished interval",interval,"of",ScreenPrintInterval,sep=" "))
}
...
print("starting to analyze match data")
MatchTypeToSum <- "road"
NumberMatches<- integer(nFail)
for (i in 1:nFail){
  pattern <- ifelse(MatchTypeToSum=="road", paste("ID",IDs[i],"-r",sep=""), paste("ID",IDs[i],"-stream",sep=""))
  matchFile <- list.files(path="/Users/MM/Documents/Research/Climate_Scour_Local/R_data/Output", pattern = pattern)[1]
  load(paste("/Users/MM/Documents/Research/Climate_Scour_Local/R_data/Output/",matchFile,sep=""))
