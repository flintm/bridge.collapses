...
for (i in 1:nP){
  ...
  df <- switch(pT,
               ...
               pUSGSLP3d = BridgesDataFrame[,c("LABEL","T_FAIL_DAILY_LP3_USGS","T_MAX_DAILY_LP3_USGS","T_MAX_PRE_FAIL_DAILY_LP3_USGS")],
               pVICgHEC  = BridgesDataFrame[,c("LABEL","T_FAILPM2_HEC_DVICG","T_MAXPREFAILPM2_HEC_DVICG","T_MAX_HEC_DVICG")]
               )
  title <- switch(pT,
                  pHECd     = "BULLETIN 17B - DAILY MEAN",
                  pHECip    = "BULLETIN 17B - INST/PEAK",
                  pHECdip   = "BULLETIN 17B - DAILY MEAN & INST/PEAK",
                  pUSGSLP3d = "LOG-PEARSON III - DAILY MEAN",
                  pVICgHEC  = "BULLETIN 17B - DAYMET-VIC"
                  )
  ...
  if (pT != "pHECdip"){
    ...
    colnames(df)[2:7] <- c("FAILD","MAXD","PREFMAXD", "FAILIP", "MAXIP", "PREFMAXIP")
    df.melt <- melt(df, id.names = "LABEL", value.name = "T", variable.name = "GROUP")
    ...
    if (any(df.line$FAILD >= Breaks[length(Breaks)] | df.line$MAXD >= Breaks[length(Breaks)] | df.line$PREFMAXD >= Breaks[length(Breaks)] |
            df.line$FAILIP >= Breaks[length(Breaks)] | df.line$MAXIP >= Breaks[length(Breaks)] | df.line$PREFMAXIP >= Breaks[length(Breaks)])){
      print("T larger than max break")
  ...
  if (SHOW_Q100_SYMB == TRUE & LEGEND[i] == TRUE){
    #     df.Q100 <- data.frame(T = c(1,1), LABEL = rep(limits[1],2), GROUP = c("Q100", "Q500"))
    #     p[[pT]] <- p[[pT]] + geom_point(data = df.Q100, aes(shape = GROUP, size = GROUP), alpha = 0, color = "white")
 ...
 p[[pT]] <- p[[pT]] + 
   ...
   geom_point(data = subset(df.melt2, !(TYPE %in% c("Q100","Q500"))), aes(shape = TYPE, size = TYPE, alpha = BOOL_KNOWN_FAIL_DATE, color = FAIL_CAUS_CODE)) 
 if (SHOW_Q100_SYMB == TRUE) p[[pT]] <- p[[pT]] + geom_point(data = subset(df.melt2, TYPE %in% c("Q100","Q500")), aes(shape = TYPE, size = TYPE), color = "white", alpha = 0) 
 if (T_REPORT == TRUE){
   p[[pT]] <- p[[pT]] + 
     geom_point(data = subset(df.report, !is.na(T)),aes(shape = GROUP, size = GROUP, alpha = BOOL_KNOWN_FAIL_DATE)) +  
     geom_linerange(data = subset(df.report, !is.na(ymin)), aes(x = LABEL, ymin = ymin, ymax = ymax), size = 0.25) 
 ...
 if(SEGMENTS==TRUE & pT != "pHECdip"){
   p[[pT]] <- p[[pT]] +
     geom_linerange(aes(ymin = PREFMAX, ymax = MAX, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty="dashed") +
     geom_linerange(aes(ymin = FAIL, ymax = PREFMAX, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty = "dotted")
 }
 if(SEGMENTS==TRUE & pT == "pHECdip"){
   p[[pT]] <- p[[pT]] +
     geom_linerange(aes(ymin = PREFMAXD, ymax = MAXD, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty="dashed") +
     geom_linerange(aes(ymin = FAILD, ymax = PREFMAXD, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty = "dotted") +
     geom_linerange(aes(ymin = PREFMAXIP, ymax = MAXIP, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty="dashed") +
     geom_linerange(aes(ymin = FAILIP, ymax = PREFMAXIP, color = FAIL_CAUS_CODE, alpha = BOOL_KNOWN_FAIL_DATE), size = 0.25, lty = "dotted")
 }
 ...
 p[[pT]] <- p[[pT]] +
   theme(panel.background   = element_rect(fill = "white"),
         ...
         axis.text.x   = element_text(color = "black", size = textP$reg[outputType], angle = 90, vjust = 0.5, hjust = 1, margin  = margin(0.16,0,0,0, "cm")),
...
# MakeGridPlot(grp, SCREEN = TRUE, SAVE = SAVE, SIZE = SIZE, SAVENAME = paste("ReturnPeriods",paste(plotTypes,collapse = ""),sep="-"))
return(p)
}
