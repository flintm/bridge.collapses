PlaceLogAnnotation <- function(min,max,nRows, ANNOTATE_LOC = "TL",AXES = "LOG", INDENT = rep(FALSE,nRows)){
  if(!any(ANNOTATE_LOC == c("TL","BR","TR","BL"))) warning("ANNOTATE_LOC must be [T,B][L,R] (e.g., TL = top left)", immediate. = TRUE)
  if(!any(AXES == c("LOG","LINEAR"))) warning("AXES must be LOG or LINEAR", immediate. = TRUE)
