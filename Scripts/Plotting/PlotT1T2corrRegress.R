...
if (length(TEXT) >=1){
  ...
  if (SCALE == "LINEAR") limits[1] <- min(abs(floor(limits[1]/roundVal)*roundVal - roundVal),5)
  if(SCALE == "LOG" & limits[1]<=0) limits[1] <- 0.01
...
if (SCALE == "LINEAR"){
  p <- p + coord_equal(xlim = limits, ylim = limits) +
    geom_abline(intercept = LRintercept, slope = LRslope, color = "black",size=0.25)
}
if (SCALE == "LOG" | T2l == "T_PARTIAL"){
  p <- p + scale_x_log10(limits = limits, breaks=breaks, labels = breakLabels) + scale_y_log10(limits = limits, breaks=breaks, labels = breakLabels) +
    geom_abline(intercept = LRintercept.log, slope = LRslope.log, color = "black",size=0.25)
}
...
if (length(TEXT) >=1){
  ...
  df.text <- PlaceLogAnnotation(limits[1],limits[2],nRows, ANNOTATE_LOC = ANNOTATE_LOC, SCALE = SCALE, INDENT = INDENT)
  text <- character()
  for (i in TEXT){
    text[i] <- switch(i,
...
p <- p +
  theme(panel.background   = element_rect(fill = "white", color = "gray",size=0.25),
